...
public class EventDao extends Dao<Event> {
    ...
    public LinkedHashMap<Long, HashMap<String, String>> getFinishedAndUnassignedEventCountsInCampaignsAsMap(Long pkId) {
        String hql = "select ev.campaign,sum(case when ev.user is not null then 1 else 0 end),sum(case when ev.finalComment is not null then 1 else 0 end) from Event ev ";

        hql += " where ev.cabinet.pkId=:pkId ";
