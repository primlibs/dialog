        ...
        <div class="row form-group">
            <form style="float: left;" class="form-inline" action="<c:url value="/Event/addObserver" />" method="post">
                <c:if test="${not empty cabinetUserList}">
                    <div class="form-group">
                        <select name="cabinetUserId"  class="form-control">
            ...
            <c:if test="${not empty observerList}">
                <c:forEach var="observer" items="${observerList}">
                    <a href="<c:url value="/Event/delObserver?campaignObserverId=${observer.campaignObserverId}&campaignId=${param.campaignId}"/>" style="float: right;margin-right: 10px;" class="btn btn-warning deletinghref" role="button">${observer.user.surname} ${observer.user.name}</a>
                </c:forEach>
            </c:if>         
        </div>
